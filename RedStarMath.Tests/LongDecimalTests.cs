...
public class LongDecimalTests
{
	...
	public void TestToByteArray()
	{
		...
		for (var i = 0; i < 250000; i++)
		{
			...
			LongDecimal lm3 = new(lm.ToByteArray(order), order);
			Assert.IsTrue(LongDecimal.IsNaN(lm) && LongDecimal.IsNaN(lm2) && LongDecimal.IsNaN(lm3)
				|| lm.Equals(lm2) && lm.Equals(lm3));
		}
		int RandomOrder() => random.Next(2) * 2 - 1;
	}
